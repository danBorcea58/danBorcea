...
def toroydPreProcessing(section, finalTheta, GEO, CH, np):
    thetaInitial = np.arctan2(CH["refCoor"][2],CH["refCoor"][1])+np.pi*2/CH["number"]*(section) 
    if section == 0:
    ...
    while distance2 >= 0:
        ...
        sigma += dsigma
        x += dx
        distance2 = -GEO["radius"]["toroydInit"]-GEO["thickness"]["external"] + GEO["pos"]["end"] - x
        
        dist2 = -(GEO["spline"]["tubeThickness"](2*np.pi))*2 - GEO["radius"]["toroydEnd"] - GEO["spline"]["tubeThickness"](2*np.pi) + GEO["pos"]["end"] \
            -GEO["radius"]["toroydInit"]*(1+0*np.cos(GEO["angle"]["channel"]["end"]))-GEO["radius"]["toroydEnd"]
    ...
    if section == 0:
        ...
        sigmaVecEnd = np.pi*2/CH["number"]*(section+1)-GEO["thickness"]["interWall"]*2/(GEO["spline"]["profile"]["fun"](GEO["pos"]["end"]-GEO["length"]["DELTA"]))
        ...
        sigmaVecEnd = np.pi*2/CH["number"]+(GEO["sigmaRange"]-np.pi*2/CH["number"])/(CH["number"]-1)*(section)-GEO["thickness"]["interWall"]*2/(GEO["spline"]["profile"]["fun"](GEO["pos"]["end"]-GEO["length"]["DELTA"]))
        sigmaVec = sigmaVecStart + s*(sigmaVecEnd-sigmaVecStart)
    
    thetaVecEnd = thetaFinalFinal-GEO["thickness"]["interWall"]*2/(GEO["spline"]["profile"]["fun"](GEO["pos"]["end"]-GEO["length"]["DELTA"]))
    ...
    for step in range(steps+1):  
        ...
        if step == 0:
            ...
            for i in range(len(startingNodes)):
                ...
                otherAngle = intersection(GEO, rCentre3, xCentre3, distance, np, fsolve)
