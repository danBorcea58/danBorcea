...
def meshConstruction(SEC, GEO, PARAM, INPUT, i, angle, WL, flag):
    ...
    if flag == "infill":
        ...
        if flag == "mesh": 
            ...
            r = refine_edges(r, factor=10, edge_fraction=0.05)           # Refine the mesh at the edges.
        else:
        ...
        if flag == "mesh" or ("z" not in GEO):
            z = refine_edges(np.linspace(0, GEO["waveLength"], n_z), factor=10, edge_fraction=0.05)
        else:
